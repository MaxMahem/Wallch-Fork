/****************************************************************************
** Meta object code from reading C++ file 'MyCameraWindow.h'
**
** Created: Tue Aug 23 03:06:44 2011
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "MyCameraWindow.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'MyCameraWindow.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_MyCameraWindow[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      25,   16,   15,   15, 0x05,

 // slots: signature, parameters, type, tag, flags
      44,   15,   15,   15, 0x08,
      57,   15,   15,   15, 0x08,
      72,   15,   15,   15, 0x08,
      84,   15,   15,   15, 0x08,
      97,   15,   15,   15, 0x08,
     109,   15,   15,   15, 0x08,
     123,   15,   15,   15, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_MyCameraWindow[] = {
    "MyCameraWindow\0\0img_path\0save_path(QString)\0"
    "take_image()\0image_update()\0close_now()\0"
    "save_image()\0new_image()\0update_text()\0"
    "update_color()\0"
};

const QMetaObject MyCameraWindow::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_MyCameraWindow,
      qt_meta_data_MyCameraWindow, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &MyCameraWindow::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *MyCameraWindow::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *MyCameraWindow::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_MyCameraWindow))
        return static_cast<void*>(const_cast< MyCameraWindow*>(this));
    return QWidget::qt_metacast(_clname);
}

int MyCameraWindow::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: save_path((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 1: take_image(); break;
        case 2: image_update(); break;
        case 3: close_now(); break;
        case 4: save_image(); break;
        case 5: new_image(); break;
        case 6: update_text(); break;
        case 7: update_color(); break;
        default: ;
        }
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void MyCameraWindow::save_path(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
