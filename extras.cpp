...
Extras::Extras(QWidget *parent) : QDialog(parent), ui(new Ui::Extras) {
    ui->setupUi(this);
    ...
    if (settings.value("liveEarthFileName").toString() == "world_sunlight_map_hemisphere.jpg")
        ui->liveEarthRadioHemisphere->setChecked(true);
    else
        ui->liveEarthRadioRectangular->setChecked(true);

    if (ui->image_from_disk_lock->isChecked()) {
...
void Extras::closeEvent( QCloseEvent * ) {
    ...
    if (ui->liveEarthRadioHemisphere->isChecked())
        settings.setValue("liveEarthFileName", "world_sunlight_map_hemisphere.jpg");
    else
        settings.setValue("liveEarthFileName", "world_sunlight_map_rectangular.jpg");
...
void Extras::on_closeButton_clicked() { close(); }
...
void Extras::on_liveEarthActivatePushButton_clicked() {
    Q_EMIT start_live_earth();

    // deactivate buttons
    ui->liveEarthDeactivatePushButton->setEnabled(true);
    ui->liveEarthActivatePushButton->setEnabled(false);
    ui->liveEarthRadioHemisphere->setEnabled(false);
    ui->liveEarthRadioRectangular->setEnabled(false);
}

void Extras::on_liveEarthDeactivatePushButton_clicked() {
    Q_EMIT stop_live_earth();

    // activate buttons
    ui->liveEarthDeactivatePushButton->setEnabled(false);
    ui->liveEarthActivatePushButton->setEnabled(true);
    ui->liveEarthRadioHemisphere->setEnabled(true);
    ui->liveEarthRadioRectangular->setEnabled(true);
}
